  ...
  Future handle(Task task, {Function(TransferProgress progress) callback}) {
    ...
    if (task is FileTask &&
      ...
      for (var workerIsolate in workingIsolates) {
        Fimber.d('Worker: handle: CancelFileTask: $workerIsolate');
...
//      Fimber.d('Worker: receivePort: $message');
...
//          Fimber.d('... but not callback for taskId=${message.taskId}');
        }
        ...
        Fimber.d('... CancelFileTask this=$this');

        return;
      } else if (message is SendPort) {
        Fimber.d('... SendPort this=$this');
        ...
        Fimber.d('... SendPort this String=$message');
//        completer.complete(this);
...
//        this._runNextTask();
        ...
        Fimber.d('... WorkerResult result=${message.result}, this=$this');
  ...
  Future performTask(Task task,
    ...
    Fimber.d('Worker: performTask $task');
    if (isClosed) throw StateError('This WorkerIsolate is closed.');

    if (task is FileTask &&
        (task.actionType == ActionType.cancelUpload ||
            task.actionType == ActionType.cancelDownload)) {
      Fimber.d('Worker: performTask _sendPort.send of CancelFileTask');
      _sendPort.send(task);
